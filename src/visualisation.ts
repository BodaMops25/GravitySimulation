import { BarnesHutRootType, Point, SectorDirects } from "./barnes-hun";
import { Camera } from "./camera";
import { Vec } from "./helpers";

const colorsList = ['red', 'green', 'blue', 'gray', 'darkGray', 'purple']
const colors = [
  "#FF573390", // Bright Red-Orange
  "#33FF5790", // Bright Green
  "#3357FF90", // Bright Blue
  "#FF33A190", // Bright Pink
  "#FF8C3390", // Orange
  "#33FFF290", // Aqua
  "#FF333390", // Red
  "#9933FF90", // Purple
  "#33FF8C90", // Mint Green
  "#3333FF90", // Royal Blue
  "#FF573390", // Coral
  "#57FF3390", // Lime Green
  "#F1C40F90", // Yellow
  "#2980B990", // Navy Blue
  "#8E44AD90", // Plum
  "#D3540090", // Rust
  "#1ABC9C90", // Turquoise
  "#2ECC7190", // Green
  "#3498DB90", // Light Blue
  "#E74C3C90", // Bright Red
  "#9B59B690", // Lavender
  "#34495E90", // Dark Blue-Grey
  "#E67E2290", // Orange-Yellow
  "#BDC3C790", // Light Grey
  "#95A5A690", // Grey
  "#2C3E5090", // Charcoal
  "#7F8C8D90", // Steel Grey
  "#16A08590", // Teal
  "#C0392B90", // Maroon
  "#F39C1290"  // Amber
];

export function drawGrid(pos: Vec, width: number, height: number, {
  camera,
  lineWidth=1,
  strokeStyle='black',
  points,
  pointRadius=5,
  pointColor='black',
  pointLineWidth,
  pointStrokeStyle,
  pointCallback
}: {
  camera: Camera,
  lineWidth?: number,
  strokeStyle?: string,
  points?: Point[],
  pointRadius?: number,
  pointColor?: string,
  pointLineWidth?: number,
  pointStrokeStyle?: string,
  pointCallback?: (ctx: CanvasRenderingContext2D, pointX: number, pointY: number, point: Point) => void
}) {

  /* if(points) {
    for(const point of points) {

      const pointX = x + point.pos.x,
          pointY = y + point.pos.y
      
      c.beginPath()
      c.arc(pointX, pointY, point.radius || pointRadius, 0, Math.PI*2)
      c.fillStyle = point.color || pointColor
      c.fill()

      if(
        point.strokeStyle || point.lineWidth ||
        pointStrokeStyle || pointLineWidth
      ) {
        c.lineWidth = point.lineWidth || pointLineWidth || 1
        c.strokeStyle = point.strokeStyle || pointStrokeStyle || 'black'
        c.stroke()
      }

      if(pointCallback) {
        pointCallback(c, pointX, pointY, point)
      }
    }
  } */

  const c = camera.canvasHelper.ctx
  if(!c) {
    console.warn('No canvas context 2d');
    return
  }

  c.lineWidth = lineWidth
  c.strokeStyle = strokeStyle

  const {x, y} = camera.map2CameraPos(pos),
        cnvsWidth = width * camera.scale,
        cnvsHeight = height * camera.scale

  c.beginPath()
  c.rect(x, y, cnvsWidth, cnvsHeight)
  c.stroke()

  c.beginPath()
  c.moveTo(x, y + cnvsHeight/2)
  c.lineTo(x + cnvsWidth, y + cnvsHeight/2)
  c.stroke()

  c.beginPath()
  c.moveTo(x + cnvsWidth/2, y)
  c.lineTo(x + cnvsWidth/2, y + cnvsHeight)
  c.stroke()
}

export function visualizeParticlesTreeInArea(particlesTree: BarnesHutRootType, {
  camera,
  lineWidth,
  strokeStyle,
  pointRadius,
  pointColor,
  pointLineWidth,
  pointStrokeStyle,
  pointCallback,
  pos, width, height
}: {
  camera: Camera,
  lineWidth?: number,
  strokeStyle?: string,
  pointRadius?: number,
  pointColor?: string,
  pointLineWidth?: number,
  pointStrokeStyle?: string,
  pointCallback?: (ctx: CanvasRenderingContext2D, pointX: number, pointY: number, point: Point) => void,
  pos: Vec, width: number, height: number
}, offsetX=0, offsetY=0) {

  const points = [],
    gridWidth = particlesTree.data.scale * width,
    gridHeight = particlesTree.data.scale * height

  for(const sectorKey in particlesTree.sectors) {
    const sector = particlesTree.sectors[sectorKey as SectorDirects]

    let additionalOffsetX = 0,
      additionalOffsetY = 0

    if(sectorKey?.includes('e')) additionalOffsetX = particlesTree.data.scale / 2
    if(sectorKey?.includes('s')) additionalOffsetY = particlesTree.data.scale / 2

    // if(sector !== undefined && sector.sectors === undefined) points.push({
    //   ...sector,
    //   x: ((sector as Point).pos.x - offsetX) / particlesTree.data.scale * gridWidth,
    //   y: ((sector as Point).pos.y - offsetY) / particlesTree.data.scale * gridHeight
    // })
    if((sector as BarnesHutRootType).sectors !== undefined) {

      visualizeParticlesTreeInArea(sector as BarnesHutRootType, {
        camera,
        lineWidth,
        strokeStyle,
        pointRadius,
        pointColor,
        pointLineWidth,
        pointStrokeStyle,
        pointCallback,
        pos, width, height
      }, offsetX + additionalOffsetX, offsetY + additionalOffsetY)
    }
  }

  // points.push({
  //   x: (particlesTree.data.x - offsetX) / particlesTree.data.scale * gridWidth,
  //   y: (particlesTree.data.y - offsetY) / particlesTree.data.scale * gridHeight,
  //   radius: particlesTree.data.mass,
  //   color: strokeStyle,
  //   lineWidth: 1,
  //   strokeStyle: 'black',
  //   label: particlesTree.data.id
  // })

  const gridX = pos.x + width * offsetX,
      gridY = pos.y + height * offsetY

  drawGrid({
    x: pos.x + width * offsetX,
    y: pos.y + height * offsetY
  }, 
  gridWidth, gridHeight, 
  {
    camera,
    lineWidth,
    strokeStyle,
    pointRadius,
    pointColor,
    pointLineWidth,
    pointStrokeStyle,
    pointCallback
  })
}